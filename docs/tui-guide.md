...
After `dev-feature` launches the stack, it enters an interactive terminal dashboard. The dashboard has four modes: Overview (default), Log View, Diagnose, and Credentials. You can switch between them with the keybindings described below.
...
Pressing `Ctrl+C` twice (or sending `SIGKILL`) bypasses the grace period and terminates processes immediately. On the next launch, `dev-feature` detects orphaned PIDs from the previous session and kills them before starting new processes.
