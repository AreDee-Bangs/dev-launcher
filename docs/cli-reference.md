# dev-feature CLI Reference

`dev-feature` is a multi-product stack launcher that creates and manages git worktrees for
...
| `--logs-dir <PATH>` | Override the log directory. Each service writes a `.log` file there. Default: `/tmp/dev-feature-logs/{workspace-hash}/`. |
...
dev-feature --copilot-branch fix/auth-bug
...
dev-feature --workspace 4d448a3f

# Use a local directory you already have checked out
dev-feature --copilot-worktree ~/dev/filigran/filigran-copilot

# Pin Copilot to a specific commit for regression testing
dev-feature --copilot-commit abc1234def5678

# Full stack minus both frontends (faster boot, API-only testing)
dev-feature --no-opencti-front --no-openaev-front

# Custom log directory
dev-feature --logs-dir /var/log/dev-feature
```
