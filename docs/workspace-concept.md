...
This document explains how `dev-feature` organizes feature branches into isolated workspaces.
...
When you launch a feature, `dev-feature` adds a worktree directory next to each main clone:
...
| Service | Default (`./dev.sh`) | Workspace (`dev-feature`) |
|---|---|---|
...
**Listed**: on subsequent runs without explicit flags, `dev-feature` shows an interactive selector of existing workspaces, sorted most-recent-first. Each row shows product labels, branch names, and the last-used date.
