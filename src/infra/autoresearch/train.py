...
def verify_env():
    if sys.platform == "darwin":
        if not torch.backends.mps.is_available():
            print("WARNING: MPS not available on this Mac — falling back to CPU.")
        else:
            print("Environment verified: macOS with Metal (MPS) available.")
    elif torch.cuda.is_available():
        print(f"Environment verified: CUDA device {torch.cuda.get_device_name(0)}")
    else:
...
# CTI eval: sample completions from a SIGMA rule seed prompt and score field coverage.
# A "valid" sample contains ≥3 of the 6 canonical SIGMA fields.
_SIGMA_FIELDS = ("title:", "id:", "status:", "logsource:", "detection:", "condition:")
_SIGMA_SEED   = "title:"
_CTI_SAMPLES  = 16

def _sigma_validity_rate(mdl, tok) -> float:
    try:
        seed_ids = tok.encode(_SIGMA_SEED)
        if not seed_ids:
            return 0.0
    except Exception:
        return 0.0
    ...
    for _ in range(_CTI_SAMPLES):
        try:
            ...
            for _ in range(300):
                ...
                # Guard against NaN/Inf (common on undertrained models)
                if not torch.isfinite(raw).all():
                    gen = None
                    break
                probs = torch.softmax(raw / 0.9, dim=-1)
            ...
            continue
        if sum(1 for f in _SIGMA_FIELDS if f in text) >= 3:
            valid += 1
    return valid / _CTI_SAMPLES

# Final summary — printed before sigma eval so val_bpb always reaches the log.
t_end = time.time()
...
# CTI eval runs after core metrics are already logged — crash here is non-fatal.
try:
    sigma_validity_rate = _sigma_validity_rate(model, tokenizer)
    print(f"sigma_validity_rate:    {sigma_validity_rate:.4f}")
except Exception as _cti_err:
    print(f"sigma_validity_rate:    0.0000  # eval failed: {_cti_err}")
