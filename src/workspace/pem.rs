    ...
    for dir in search_dirs {
        ...
        for entry in entries.flatten() {
            ...
            for def in PRODUCT_PEMS {
                ...
                if candidates
                    .iter()
                    .any(|c| c.path == path && c.env_key == def.env_key)
                {
                    continue;
                }
...
pub fn run_pem_selector(candidates: &mut Vec<PemCandidate>, stopping: &Arc<AtomicBool>) {
    ...
    let render = |items: &Vec<PemCandidate>, cursor: usize| {
        print!(
            "  {BOLD}License PEM files found{R}  {DIM}↑↓ navigate   Space toggle   Enter confirm   Esc back to menu{R}\r\n\r\n"
        );
    ...
    println!("  {BOLD}License PEM files found{R}  {DIM}↑↓ navigate   Space toggle   Enter confirm   Esc back to menu{R}");
    println!();
    let mut last_product = "";
...
pub fn inject_selected_pems(candidates: &[PemCandidate], ws_env_dir: &Path) {
    for c in candidates.iter().filter(|c| c.selected) {
        ...
        println!(
            "  {GRN}✓{R}  {} → {BOLD}{}{R}  {DIM}({}){R}",
            c.display, c.env_key, c.product_label,
        );
    }
}
