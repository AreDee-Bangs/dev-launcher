...
impl CredEntry {
    fn entry(product: &'static str, label: &'static str, value: impl Into<String>) -> Self {
        Self { product, label, value: value.into(), note: false }
    }
    fn note(product: &'static str, text: &'static str) -> Self {
        Self { product, label: text, value: String::new(), note: true }
    }
}
...
pub fn gather_credentials(ws_env_dir: &Path, paths: &Paths) -> Vec<CredEntry> {
    ...
    if paths.langfuse.is_dir() {
        ...
        out.push(CredEntry::note( "Copilot → Settings → Langfuse", "Enable tracing: open Copilot → Settings → Langfuse and enter:"));
        out.push(CredEntry::entry("Copilot → Settings → Langfuse", "Host",       url));
        out.push(CredEntry::entry("Copilot → Settings → Langfuse", "Public key", pk));
        out.push(CredEntry::entry("Copilot → Settings → Langfuse", "Secret key", sk));
    }
