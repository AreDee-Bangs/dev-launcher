...
pub fn init(recipes_dir: &Path) {
    ...
    crate::llog!("[RECIPES] loaded {} recipe(s) from {}", recipes.len(), recipes_dir.display());
...
fn apply_inner(
    ...
    for recipe in recipes {
        ...
        let all_match = recipe.matches.iter().all(|block| {
            let any_ok = block.any_of.as_ref().map_or(true, |needles| {
                needles.iter().any(|n| {
                    let nl = n.to_lowercase();
                    log_lines.iter().any(|l| l.to_lowercase().contains(&nl))
                })
            });
            let all_ok = block.all_of.as_ref().map_or(true, |needles| {
                needles.iter().all(|n| {
                    let nl = n.to_lowercase();
                    log_lines.iter().any(|l| l.to_lowercase().contains(&nl))
                })
        ...
        let pip    = backend_dir.join(".venv/bin/pip").to_string_lossy().into_owned();
        let python = backend_dir.join(".venv/bin/python").to_string_lossy().into_owned();
        ...
        let finding = if let Some(fix) = &recipe.fix {
            let steps: Vec<FixStep> = fix.steps.iter().map(|step| {
